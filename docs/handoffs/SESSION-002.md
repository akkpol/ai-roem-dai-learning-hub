...
- Added a credential-separated PostgreSQL platform boundary, reviewed Drizzle migration, application role contract, and guarded integration reset.
...
- Updated GitHub Actions to start PostgreSQL 18.4, bootstrap the application role, apply migrations with the migration credential, run PostgreSQL integration tests with both credentials, build without database URLs, and smoke the production server through the application URL including exact readiness JSON.
...
- `npm test`: exit 0; 7 test files and 20 tests passed.
...
- Local Docker PostgreSQL bootstrap, migration, integration suite, and production readiness smoke: **NOT RUN**. The `docker` executable is not installed in this environment.
- GitHub Actions PostgreSQL migration, integration suite, and smoke gate: **NOT RUN**. The workflow is configured, but this coding session did not create a PR or push a run; no CI URL or result exists yet.
- Neon temporary branch, remote migration, integration suite, privilege/schema inspection, and branch cleanup: **NOT RUN**. No Neon branch or database was created.
...
- The CI PostgreSQL bootstrap and integration commands are configured but unobserved here because local Docker is unavailable and no remote CI run was triggered.
