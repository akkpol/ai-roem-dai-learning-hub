...
async function json(response: Response) { try { return await response.json() as unknown; } catch { return undefined; } }
function message(body: unknown, fallback: string) { return (body as { message?: string } | undefined)?.message ?? fallback; }
...
export function OrganizationWorkspace({ organizationId }: { organizationId: string }) {
  ...
  const load = useCallback(async () => { setState((current) => ({ ...current, loading: true, message: null })); try { const response = await fetch(`/api/organizations/${organizationId}`); const body = await json(response); if (!response.ok) { setState({ value: null, message: message(body, "ไม่สามารถโหลดองค์กรได้"), loading: false, status: response.status }); return; } setState({ value: body as OrganizationWorkspaceDto, message: null, loading: false }); } catch { setState({ value: null, message: "เชื่อมต่อไม่สำเร็จ กรุณาลองอีกครั้ง", loading: false }); } }, [organizationId]);
  useEffect(() => { const timeout = window.setTimeout(() => void load(), 0); return () => window.clearTimeout(timeout); }, [load]);
